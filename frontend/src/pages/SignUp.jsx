...
const TEXTS = ['Experience the Future of AI', 'Your Smart AI Companion', 'Powered by Intelligence']
...
function SignUp() {
  ...
  const handleSignUp = async (e) => {
    // Implement sign-up logic here (e.g., API call to backend)
    e.preventDefault()
    setErr('')
    setLoading(true)
    try{
      ...
      setUserData(null)
      setLoading(false)
      setErr(e?.response?.data?.message || 'An error occurred during sign-up.')
    }
  }
  ...
  return (
    ...
    >
      ...
      {/* MAIN CARD */}
      <form className="float-card relative z-10 w-[92%] max-w-xl p-7 rounded-[40px] overflow-hidden bg-white/10 backdrop-blur-md border border-white/20 shadow-[0_0_60px_rgba(0,0,0,0.6)] hover:scale-[1.01] transition-all duration-500 cursor-pointer" onSubmit={handleSignUp}>
        ...
        <div className='text-center mb-4'>
          <h1 className='text-5xl font-extrabold tracking-wide text-white'>
            Virtual
            <span className='bg-linear-to-r from-pink-400 via-purple-400 to-cyan-400 bg-clip-text text-transparent'> Assistant</span>
          </h1>
        ...
        {/* NAME */}
        <div className='input-field hover:border-pink-400 hover:shadow-[0_0_20px_rgba(236,72,153,0.4)] mb-4'>
          <FaUser className='text-pink-400 text-2xl shrink-0' />
          <input type='text' placeholder='Enter your name' className='input-base' required onChange={(e) => setName(e.target.value)} value = {name} />
        </div>
        ...
        <div className='input-field hover:border-cyan-400 hover:shadow-[0_0_20px_rgba(34,211,238,0.4)] mb-4'>
          ...
          <input type='email' placeholder='Enter your email' className='input-base' required onChange={(e) => setEmail(e.target.value)} value = {email} />
        </div>

        {/* PASSWORD */}
        <div className='input-field hover:border-purple-700 hover:shadow-[0_0_20px_rgba(168,85,247,0.9)] mb-5 relative'>

          <FaLock className='text-purple-700 text-2xl shrink-0' />

          <input type={showPassword ? 'text' : 'password'} 
          minLength={6}
          placeholder='Enter your password' className='input-base' required onChange={(e) => setPassword(e.target.value)} value = {password} />

          {!showPassword && <PiEyeClosedBold className='absolute top-3.5 right-5 text-white w-7 h-7 cursor-pointer' onClick={() => setShowPassword(true)}/>}
          {showPassword && <PiEyeFill className='absolute top-3.5 right-5 text-white w-7 h-7 cursor-pointer' onClick={() => setShowPassword(false)}/>}
    
        </div>

        {err.length > 0 && <p className = "text-red-400 text-center font-semibold text-lg mb-3">* {err}</p>}
        ...
        <button className='relative overflow-hidden w-full py-4 rounded-2xl text-xl font-bold tracking-wide text-white bg-linear-to-r from-pink-500 via-purple-500 to-cyan-500 hover:scale-105 transition-all duration-300 shadow-[0_0_50px_rgba(168,85,247,0.5)] hover:shadow-[0_0_80px_rgba(168,85,247,0.8)]' disabled={loading}>
          <span className='relative z-10'>{loading ? "Loading..." : "Create Account"}</span>
          <div className='absolute top-0 left-full w-full h-full bg-white/20 skew-x-12 animate-[shine_3s_linear_infinite]'></div>
        </button>

        {/* LOGIN */}
        <p className='text-center text-gray-300 mt-5 text-lg' onClick={() => navigate('/signin')}>
          Already have an account?
          <span className='text-pink-400 hover:text-cyan-400 cursor-pointer ml-2 transition-all duration-300 hover:underline'>Sign In</span>
        </p>
